...
# Technologies Used
When downloading the CSV's from the census website the data was long as opposed to wide. I needed to manipulate and clean the data so that all counties and cities where on a master sheet(fact tables). I did that using Excel. From there I created my dimension tables to build a data model. This included a a costck demographic table to filter from as well as a latitude and longitude table to plot counties and cities. Once all those tables were created I loaded everything into Tableau and performed my EDA. After I found some great insights I created a dashboard story in tableau.
